  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-if="row.state===1"  type="primary" size="mini" @click="ZC(row)">
            自查
          </el-button>
          <el-button v-if="row.state===3"  type="primary" size="mini" @click="ZC(row)">
            整改
          </el-button>
          <el-button  :disabled="row.state===5?false:true" size="mini" type="danger" @click="JS(row)">
            结束
          </el-button>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      ZC(row){
        this.$confirm('确认自查【'+row.id+'】吗','提示',{
          confirmButtonText:'确认',
          cancelButtonText:'取消',
          type:'warning'
        }).then(()=>{

          this.$router.push({ name:'executiveForce', query: { supervise: row }})
